...
static int emc2305_read_fan(struct device *dev, 
			 u32 attr, int channel, long *val)
{
	struct emc2305_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;

	read_fan_from_i2c(client, &data->fan[channel].target,
			  SEL_FAN(channel, REG_TACH_TARGET_HIGH),
			  SEL_FAN(channel, REG_TACH_TARGET_LOW));
	read_fan_from_i2c(client, &data->fan[channel].tach,
			  SEL_FAN(channel, REG_TACH_READ_HIGH),
			  SEL_FAN(channel, REG_TACH_READ_LOW));
	*val = data->fan[channel].tach;
	return 0;	
}
			 ...
			 u32 attr, int channel, long *val)
{
	struct emc2305_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
    	struct emc2305_fan_data *fan = emc2305_update_fan(client, channel);
			  ...
			  u32 attr, int channel, long val)
{
	...
	const u8 reg_fan_setting = SEL_FAN(channel, REG_FAN_SETTING);
	int status = 0;
...
MODULE_AUTHOR("Reinhard Pfau <pfau@gdsys.de>, Glukhov Mikhail <glukhov.mikhail@depo.ru>");
